...
- 健康栏会单独统计“内幕来源是否同步”“候选中实际有内幕记录的数量”和“有合格买入的数量”，避免把数据源可用误解为每个候选都有 Form 4 证据。
...
- 技术分析同样基于本地日线快照，独立识别“上穿 20 日均线”“突破前 20 日最高收盘价”和“放量突破”（量比至少 1.5 倍）。每只标的需至少 21 个有效交易日；历史不足时明确标记，不会生成虚假信号。技术信号仅用于研究筛选与详情解释，不改变基本面总分、等级或通知规则。
- 如需立即启用技术信号，可在“小盘候选”页面执行一次`回填技术历史`：它只处理当前 A/B 候选中历史不足 21 个交易日的标的，默认请求近 35 个自然日，并沿用现有行情源、token 轮换、请求预算和回退顺序。回填不发布新市场批次、不改基本面评分、不发送通知；日常行情同步继续只补最新交易日。
