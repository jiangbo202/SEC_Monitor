...
- **上游格式漂移隔离**：Nasdaq 上市目录代码集中映射；已支持 Texas Stock Exchange 的 `F` / `TXSE` 过渡值，未来出现未识别交易所时会先隔离为非规范场所，既不会误入候选池，也不会拖垮整批日更。
...
- **有上限的自动恢复**：技术历史按标的退避重试，连续失败达到上限后进入人工处理队列，不会无限拖长后续同步；新股等自然历史不足会标为“等待样本”，不累计失败次数或制造伪告警；单只人工重试不会重跑完整工作流。
...
- **供应商级节流与退避**：Longbridge 公司资料、共识、估值和期权研究共用全局请求节拍；遇到限流时按有上限的冷却时间自动退避，避免不同研究任务同时重试形成请求风暴。
- **价格周期先修复再回放**：每日价格行为回放会限量补齐缺失 OHLC，再对已就绪标的重放规则；单只补齐失败不会丢弃其他标的结果，执行摘要会显示尝试、成功与剩余缺口。
...
任务调度、执行记录和系统健康使用一致的状态语义，避免把可继续使用的数据误报成全任务失败：

| 状态 | 含义 | 处理方式 |
| --- | --- | --- |
| 成功 | 主流程和发布均完成 | 无需处理 |
| 部分完成 | 主结果可用，仍有明确的数据缺口 | 缺口进入按标的或按来源补偿 |
| 降级完成 | 使用回退来源或排除了不可用标的 | 保留结果，同时在健康页展示原因 |
| 失败 | 主结果未发布 | 按持久化重试策略自动补偿，达到上限后等待常规定时或人工处理 |
| 等待样本 | 上市时间较短，尚不可能满足历史窗口 | 到预计样本充足后再检查，不计作供应商故障 |

同一任务、同一天、同类错误产生的自动补偿会在“执行记录”折叠为一条，并以徽标显示真实尝试次数；手动执行始终单独保留。小盘候选同步使用独立的“发现记录”展示逐阶段进度，长任务的页面请求超时不会取消后台工作流，可根据持续更新的心跳判断任务仍在运行。
