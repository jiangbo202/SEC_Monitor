...
- 调度任务 `sqlite_backup` 默认每天 `03:15`（按“调度任务”页面显示的时区）执行。它通过 SQLite `VACUUM INTO` 生成独立快照，先分别校验 `sec_monitor` 与 `small_cap` 的临时文件，再一起发布为完整备份组；任一阶段失败都会删除临时/半套文件。系统健康页的“恢复演练”会把最新完整备份对复制到临时隔离目录，以只读方式执行 SQLite `integrity_check` 并校验关键业务表；不会覆盖当前数据库或保留临时副本，结果会记录在本地。当前备份仍在本机 Docker volume 中；异地备份接入要求见 [备份与恢复边界](docs/operations/backup-and-recovery.md)。
...
- “系统健康 / 运行摘要与待办”提供不消耗外部接口额度的本地运行报告：聚合任务失败/部分完成/卡住、超时未运行、SEC 自动重试队列、暂缓标的、失败/死信通知批次、行情 Provider、备份过期/不完整及恢复演练异常，并附带直达处理入口。可手动发送运行摘要到 Telegram；调度任务 `operational_health_notification_sync` 默认关闭，启用后每天 `11:00` 仅在存在异常时推送，且相同异常在 12 小时内不会重复打扰。
...
- `ipo.longbridge_listing_verification_enabled`：启用 SEC ticker-only 项目的 Longbridge 自动上市确认，默认开启。
- `ipo.longbridge_listing_request_budget`：每次 IPO 同步最多请求的 Longbridge 公司概览数，默认 `20`。
- `ipo.longbridge_listing_recheck_hours`：Longbridge 未确认或临时失败后的复查间隔，默认 `24` 小时。
- `ipo.longbridge_calendar_enabled`：是否在 IPO 同步中缓存 Longbridge 美股 IPO 日历，默认开启。
- `ipo.longbridge_calendar_lookback_days` / `ipo.longbridge_calendar_lookahead_days`：IPO 日历的回看和前瞻窗口，默认 `14` / `30` 天。
- `ipo.longbridge_calendar_max_pages`：单次读取 Longbridge IPO 日历的最大分页数，默认 `5`。
...
- `IPO 日历（Longbridge）`：展示扫描时缓存的美股 IPO 日历，可按公司或标的筛选。日历是发现和补全来源；仅在公司名严格匹配 SEC 候选时补充预计上市日和标的，不会取代 SEC 的 EFFECT、424B4、RW 生命周期判断。
- `IPO进行中`统计不包含已定价、已上市、撤回/终止项目。
- 支持导出 IPO 公司 CSV 和 IPO 文件 CSV。
- SEC 上市公司映射匹配后，系统记录最终 Ticker、交易所和首次确认上市时间；该时间不是实际首日交易日期。若 SEC 已匹配到 Ticker 但交易所暂缺，且已配置 Longbridge 凭据，IPO 同步会自动查询 Longbridge 公司概览，以市场字段二次确认上市并补充实际上市日期。
- IPO 同步会先读取 SEC 的上市公司映射。只有同时具备 Ticker 与交易所的公司才视为已确认上市；其后出现的 S-1/F-1 注册文件不会再次进入 IPO 候选、回补多年历史文件或发送普通 IPO 通知。仅有 Ticker 的 `listing_pending` 公司会自动进入 Longbridge 二次确认队列；未查询到、限流或上游失败时维持待确认状态，不会误判为未上市。
