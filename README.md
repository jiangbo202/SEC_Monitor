...
- **候选全集与交易机会分离**：今日决策和策略观察池统一展示可行动、仅研究、阻断三类候选；即使数据管线健康，只要没有候选通过交易门槛，页面就会明确限制新交易计划，而不是把“没有可行动候选”误写成“没有数据”或“尚未扫描”。
- **面向处置的运行健康**：系统健康与 Dashboard 使用一致的异常口径，直接说明影响范围、自动重试或人工确认方式及处理入口；内部任务名、请求追踪标识和提供方错误细节默认转换为业务语言或隐藏。
- **交易员友好的响应式工作台**：窄屏下左侧导航切换为可访问的抽屉，Dashboard 行动队列、运行待办以及 SEC、内幕交易、监控标的等高密度表格会切换为移动卡片，避免关键数字和状态相互遮挡。
...
结合大盘指数、市场温度、行业 ETF 强弱和宏观事件复核候选，避免只依据单一总分做判断。页面保留完整 A/B 候选全集，并同时展示“可行动 / 仅研究 / 阻断”分布与每个标的的研究门槛，避免过滤条件造成“当前没有候选”的错误印象。
...
集中显示数据源、数据库、同步、调度、通知和备份状态。基础健康与运行报告合并为统一待办，每项异常说明研究或运行影响、自动恢复边界和人工处理入口；技术诊断信息仍保留在执行记录中，不占用交易员的首屏注意力。
...
- **时点明确的历史评估**：每条评估记录标明快照年龄、行情交易日、是否为该标的最新记录以及是否已被新版本替代；支持勾选两次评估直接比较分数、价格与技术状态变化。
- **事件级 SEC 阅读**：公告列表在已有结构化事件可用时直接展示事件类别、8-K Item、已确认事实、对研究论点的影响和下一步；未提炼的记录明确引导阅读 SEC 原文或手动运行 AI 分析。
- **宏观与市场研究**：查看大盘、行业 ETF、期货、宏观日历（含非农、CPI、PPI、PCE、FOMC 等）及机构 13F 持仓变化。
- **宏观影响路径**：宏观日历把通胀、就业、利率、住房、能源等事件映射到需要复核的市场因子与敏感资产，但不会在缺少可追溯预测值时自动判断利多或利空。
- **内幕交易聚合复核**：在严格的公开市场现金口径之外，按当前筛选页汇总发行人、申报人角色、买卖方向和现金净额，并突出大额或非计划内公开市场交易供优先复核。
