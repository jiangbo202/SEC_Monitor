...
- 如需立即启用技术图表，可在“小盘候选”页面执行一次`回填技术历史`：它只处理当前 A/B 候选中历史不足 50 个交易日的标的，默认请求近 120 个自然日（通常约 80 个交易日），并沿用现有行情源、token 轮换、请求预算和回退顺序。回填不发布新市场批次、不改基本面评分、不发送通知；日常行情同步继续只补最新交易日。
