    ...
    <template v-if="selected">
      <div class="result-heading">
        ...
        <div class="ai-actions">
          <el-select fit-input-width v-model="selectedAIProvider" placeholder="选择 AI 模型" style="width: 210px" :disabled="!aiProviders.length">
            <el-option v-for="provider in aiProviders" :key="provider.id" :label="`${provider.name} · ${provider.model}`" :value="provider.id" />
          </el-select>
          <el-select fit-input-width v-model="selectedAIPromptTemplate" placeholder="选择模板" style="width: 190px" :disabled="!aiPromptTemplates.length"><el-option v-for="template in aiPromptTemplates" :key="template.id" :label="template.name" :value="template.id" /></el-select>
      ...
      <el-card v-if="aiAnalyses.length" shadow="never" class="discipline ai-analysis-card">
        ...
        <el-select fit-input-width v-model="selectedAnalysisID" size="small" class="ai-analysis-select"><el-option v-for="item in aiAnalyses" :key="item.id" :label="`${item.provider_name} · ${item.model} · ${item.template_name || '历史模板'} · ${formatDate(item.requested_at)}`" :value="item.id" /></el-select>
...
function insiderCoverageUsable(row: Evaluation) { return Boolean(row.insider_coverage && row.insider_coverage.status !== 'unavailable') }
