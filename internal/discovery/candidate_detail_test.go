...
func TestGetCandidateDetailReturnsCurrentEvidence(t *testing.T) {
	...
	security := Security{CIK: "0000012345", CompanyName: "Detail Co", SIC: 2834, CatalogStatus: SecurityCatalogPublished}
	...
	if err := db.Create(&FinancialMetricSnapshot{BatchID: batch.BatchID, SecurityID: security.ID, RevenueGrowthAvailable: true, RunwayAvailable: true, QuarterlyRevenueYoYPct: 54, CashRunwayMonths: 18, QualityFlagsJSON: `["low_revenue_base"]`}).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Create(&CapitalRiskSnapshot{BatchID: batch.BatchID, SecurityID: security.ID, Kind: CapitalEventATMProgram, Active: true, BlocksA: true, BlocksB: false, Severity: CapitalRiskSeverityHigh, Reason: "ATM program active"}).Error; err != nil {
		t.Fatal(err)
	}
