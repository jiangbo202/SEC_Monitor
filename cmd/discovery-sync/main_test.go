...
type fakeSyncService struct {
	...
	incrementalResult service.DiscoverySyncResult
...
func (s *fakeSyncService) RunIncremental(context.Context) (service.DiscoverySyncResult, error) {
	s.incrementalCalled = true
	return s.incrementalResult, s.incrementalErr
}
...
func TestRunSupportsIncrementalPhase(t *testing.T) {
	t.Setenv("DISCOVERY_SYNC_PHASE", "incremental")
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	if err != nil {
		t.Fatal(err)
	}
	fake := &fakeSyncService{incrementalResult: service.DiscoverySyncResult{Status: service.DiscoverySyncStatusPublished, BatchID: "daily"}}
	var out bytes.Buffer
	err = run(context.Background(), config.Config{Discovery: config.DiscoveryConfig{Database: config.DatabaseConfig{Type: "sqlite", DSN: ":memory:"}}}, &out, syncDependencies{
		openMainDatabase:      func(config.DatabaseConfig) (*gorm.DB, error) { return db, nil },
		migrateMainDB:         database.Migrate,
	...
	if fake.runCalled || fake.marketCalled || !fake.incrementalCalled {
		t.Fatalf("run=%t incremental=%t market=%t", fake.runCalled, fake.incrementalCalled, fake.marketCalled)
	}
	if !strings.Contains(out.String(), `"batch_id":"daily"`) {
		t.Fatalf("output = %s", out.String())
	}
}
